...
export const sendMessageToAdmins = async (message: string): Promise<void> => {
  const client = await clientPromise;
  const collection = client.db("roger-bot-db").collection("users");
  const cursorUsers: FindCursor<User> = await collection.find({ "is_admin": true, "is_active": true }, {
    '_id': 0, 'telegram_id': 1
  });
...
export const sendMessageToUser = async (userTelegramId: string, message: string): Promise<void> => {
  await fetch(`https://api.telegram.org/bot${process.env.ROGER_TOKEN_BOT}/sendMessage?chat_id=${userTelegramId}&text=${message}`, { method: 'POST' })
};

export const sendHurryUpMessage = async (userTelegramId: string): Promise<void> => {
  await fetch(`https://api.telegram.org/bot${process.env.ROGER_TOKEN_BOT}/sendMessage?chat_id=${userTelegramId}&text=${getHurryUpMessage()}`, { method: 'POST' })
};

export const sendThatsItMessage = async (userTelegramId: string): Promise<void> => {
  await fetch(`https://api.telegram.org/bot${process.env.ROGER_TOKEN_BOT}/sendMessage?chat_id=${userTelegramId}&text=${getThatsItMessage()}`, { method: 'POST' })
};
...
export const sendMoodMessage = async (userTelegramId: string): Promise<TgMessage> => {
  ...
  const prevMentalRate = await mentalRateCl.findOne({ "rate": 0, "id_user": user['_id'] }, options);
