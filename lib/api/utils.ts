...
import { getTelegramId, deleteMarkupKeyboard, sendHurryUpMessage, sendThatsItMessage } from "../api/users";
...
export const checkAndDeleteMoodKeyboard = async (userId: ObjectId) => {
    ...
    const cursorMentalHours: FindCursor<MentalHours> = await mentalRateCol.aggregate(
        [
            {
                '$match': {
