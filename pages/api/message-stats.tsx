...
export default async function handler(req: NextRequest) {
    ...
    return (
      ...
      >
        ...
        <p tw="absolute -bottom-3 text-gray-400">
          * Бот показывает сообщение пользователям, если оно набирает не менее 3
          лайков и не более 25% дизлайков от общего числа оценок
        </p>
      </div>
    );
  };
      ...
      <div tw="bg-gray-900 text-gray-100 w-[40%] h-full rounded-xl p-8 flex flex-col">
        ...
        <div tw="flex flex-col h-[90%] w-full justify-center items-center">
          <div tw="flex flex-col h-20 justify-start items-center m-4">
            <p tw="text-4xl font-bold leading-none lg:text-6xl">
              {approved ? "✅" : "❌"}
            </p>
            <p tw="text-sm pt-8 text-gray-400 sm:text-base">
  ...
  const renderLink = (link: string, link_image: string, link_title: string) => {
    const trimTitle =
      link_title.length < 35 ? link_title : `${link_title.slice(0, 35)}...`;
    ...
    return (
      <div tw="flex flex-col w-full shrink text-left">
        ...
        <div tw="flex w-full justify-center relative">
          <img
            src={image}
            tw="absolute h-[256px] w-[256px]"
            style={{ filter: "blur(4px)", borderRadius: 28 }}
          />
          <img
            src={image}
            tw="h-[256px] w-[256px]"
            ...
            <div tw="bg-gray-800 text-gray-100 h-full w-full py-8 flex justify-around">
              <div tw="bg-gray-900 text-gray-100 w-[40%] h-full rounded-xl p-8 flex flex-col justify-start">
                <div tw="flex justify-between items-center w-full">
                  ...
                  <span tw="text-violet-400">
                    создано {new Date(created_date).toLocaleDateString("ru-RU")}
                  </span>
                </div>
                <div tw="flex mt-8">
                  {renderText(
                ...
                <div tw="flex mt-8">
                  {link &&
                    link_image &&
                    link_title &&
                    renderLink(link, link_image, link_title)}
                </div>
              </div>
