...
async def kb_for_stata(messages: Cursor):
    ...
    for item in messages:
        i = InlineKeyboardButton(text=str(item['text'])[:30], callback_data=cart_cb.new((str(item["_id"])), button_parameter="kb_mes"))
...
async def process_feedback_command(message: types.Message):
    ...
    user = collection_name["users"].find_one({"telegram_id": str(message.chat.id)}, {'_id': 1, "form_id": 1})
    messages = collection_name["messages"].find({"id_user": user["_id"]}, {"_id":1, "text": 1, "media_link": 1, "is_approved": 1, "image_ids": 1, "is_anonymous": 1, "created_at": 1})
...
async def send_stata(id_message: str):
    collection_name = get_database()
    message = collection_name["messages"].find_one({"_id": ObjectId(id_message)}, {"_id":1, "text": 1, "is_anonymous": 1, "created_at": 1, "is_approved": 1, "id_user": 1})
    count_times = collection_name["user_messages"].find({"id_message": message["_id"]}, {"_id": 1})
    is_approved = ""
    if (message["is_approved"] == True):
        is_approved = "да"
    else:
        is_approved = "нет"
    count_good_rated = collection_name["rate"].find({"id_message": message["_id"], "rate": True}, {"rate": 1})

    user = collection_name["users"].find_one({"_id": message["id_user"]}, {'telegram_id': 1})
...
async def process_callback_button1(message: types.Message, state:FSMContext):
    ...
    collection_name = get_database()
    admins = collection_name["users"].find({"is_admin": True, "is_active": True}, {'_id': 0, 'telegram_id': 1})
    for id in admins:
        user = collection_name["users"].find_one({'telegram_id': str(message.chat.id)}, {'telegram_username': 1})
...
async def process_start_command(message: types.Message):
    try:
        collection_name = get_database()
        user = collection_name["users"].find_one({"telegram_id" : str(message.chat.id)}, {'_id': 1, 'id_user': 1})
        id_previous_mes = collection_name['mental_rate'].find_one({"rate": 0, "id_user": user['_id']}, {'id_tg_message': 1}, sort=[("date", -1)])
        ...
        collection_name['mental_rate'].insert_one({"rate": 0, "id_user": user['_id'], "date": datetime.datetime.now(), "id_tg_message": id.message_id})
        collection_name['users'].find().close()
...
async def process_callback_button1(callback_query: types.CallbackQuery, state:FSMContext):
    ...
    try:
        collection_name = get_database()
        user = collection_name["users"].find_one({"telegram_id" : str(callback_query.from_user.id)}, {'_id': 1, 'name': 0})
        collection_name['mental_rate'].find_one_and_update({"$and": [{"id_user": user["_id"]}, {"id_tg_message": callback_query.message.message_id}]}, {"$set": {"rate": 4}})
...
async def process_callback_button1(callback_query: types.CallbackQuery, state:FSMContext):
    ...
    try:
        ...
        collection_name['mental_rate'].find_one_and_update({"$and": [{"id_user": user["_id"]}, {"id_tg_message": callback_query.message.message_id}]}, {"$set": {"rate": 3}})
        collection_name['users'].find().close()
        collection_name['mental_rate'].find().close()
    except(Exception):
        await bot.send_message(callback_query.from_user.id, "Ой, кажется, что-то пошло не так 😞 \nПовтори отправку настроения через несколько минут или напиши разработчикам через команду /feedback")
...
async def process_callback_button1(callback_query: types.CallbackQuery, state:FSMContext):
    ...
    try:
        collection_name = get_database()
        user = collection_name["users"].find_one({"telegram_id" : str(callback_query.message.chat.id)}, {'_id': 1, 'name': 0})
        collection_name['mental_rate'].find_one_and_update({"$and": [{"id_user": user["_id"]}, {"id_tg_message": callback_query.message.message_id}]}, {"$set": {"rate": 2}})
        collection_name['users'].find().close()
        collection_name['mental_rate'].find().close()
    except(Exception):
        await bot.send_message(callback_query.from_user.id, "Ой, кажется, что-то пошло не так 😞 \nПовтори отправку настроения через несколько минут или напиши разработчикам через команду /feedback")
...
async def process_callback_button1(callback_query: types.CallbackQuery, state:FSMContext):
    ...
    try:
        collection_name = get_database()
        user = collection_name["users"].find_one({"telegram_id" : str(callback_query.message.chat.id)}, {'_id': 1, 'name': 0})
        collection_name['mental_rate'].find_one_and_update({"$and": [{"id_user": user["_id"]}, {"id_tg_message": callback_query.message.message_id}]}, {"$set": {"rate": 1}})
        collection_name['users'].find().close()
        collection_name['mental_rate'].find().close()
    except(Exception):
        await bot.send_message(callback_query.from_user.id, "Ой, кажется, что-то пошло не так 😞 \nПовтори отправку настроения через несколько минут или напиши разработчикам через команду /feedback")
...
async def process_version_command(message: types.Message):
    await bot.send_message(message.chat.id, "Версия бота Роджер: 0.2.0")
...
async def create_new_user(tg_username: str, username: str, time_zone: str, telegram_id: str, user_time: str):
    try:
        collection_name = get_database()
        form_id = ObjectId()
        collection_name['users'].insert_one({"telegram_username": "@" + tg_username, "name": username, "timezone": time_zone, "is_volunteer": False, "is_banned_from_volunteering": False, "form_id": form_id, "telegram_id": telegram_id, "is_admin": False, "is_active": True, "created_at": datetime.datetime.now(), "time_to_send_messages": user_time})
...
async def get_texts_to_send_mood(arr: list, chat_id: int):
    for item in arr.get("answers_arrays"):
        ...
        if (item[0] == '*' and arr.get("is_labelled") == 1):
            ...
            if item == '*support*':
                arr = []
                user_id = collection_name["users"].find_one({"telegram_id" : str(chat_id)}, {'_id': 1, 'name': 1})
                user_messages = collection_name["user_messages"].find({"id_user": ObjectId(str(user_id["_id"]))}, {'_id': 0, 'id_message': 1})
                for item in user_messages:
                    arr.append(ObjectId(item['id_message']))
                messages = collection_name["messages"].find_one({"$and": [{'is_approved' : True}, {'id_user' : {'$ne': user_id["_id"]}}, {'_id': {"$nin": arr}}]}, {'_id': 1, 'text': 1, 'media_link': 1, 'id_user': 1, 'is_anonymous': 1, 'image_ids': 1})
                if messages!=None:
            ...
            if item == "*wait_for_answer_to_form*":
                user_id = collection_name["users"].find_one({"telegram_id" : str(chat_id)})
...
async def get_video_when_no_messages():
    response = requests.get(contentful_api_readonly_url + 'spaces/' + contenful_space_id + '/environments/master/assets?access_token=' + contenful_access_token + '&metadata.tags.sys.id[all]=videoToSendWhenNoMessages')
    answer = json.loads (response.content)
    answer = answer.get("items")
    answer = answer[random.randint(0, len(answer) - 1)]
    answer = answer.get("fields").get("file").get("url")
...
async def create_message_with_support (chat_id: int, cursor: list, username: str, user_to_send: ObjectId):
    ...
    try: 
        ...
        id_previous_mes = collection_name['user_messages'].find_one({"id_user" : user['_id']}, {"id_user": 1, "id_message": 1, "id_tg_message": 1}, sort=[("time_when_send", -1)])
        if (id_previous_mes!=None):
            rate_previous_mes = collection_name['rate'].find_one({"id_message": id_previous_mes['id_message'], 'id_user': id_previous_mes['id_user']})
        ...
        collection_name['user_messages'].insert_one({"id_user": user_to_send, "id_message": cursor["_id"], "time_when_send": datetime.datetime.now(), "id_tg_message": id_message.message_id})
        collection_name['user_messages'].find().close()
    except (Exception):
...
async def sendmes(chat_id: int):
    try:
        collection_name = get_database()
        user = collection_name["users"].find_one({"telegram_id" : str(chat_id)}, {'_id': 1, 'id_user': 1})
        id_previous_mes = collection_name['mental_rate'].find_one({"rate": 0, "id_user": user['_id']}, {'id_tg_message': 1}, sort=[("date", -1)])
        if (id_previous_mes!=None):
            await delete_keyboard(chat_id, id_previous_mes['id_tg_message'])
...
async def mental_rate_strike (chat_id: int):
    collection_name = get_database()
    user = collection_name["users"].find_one({"telegram_id": str(chat_id)}, {'_id': 1})
    all_rates = collection_name['mental_rate'].find({"$and": [{"id_user": user["_id"]}, {"rate": {"$gt": 0}}]}, sort=[("date", -1)])
...
async def ask_to_be_a_volunteer(chat_id: int):
    ...
    collection_name = get_database()
    collection_name['users'].find_one_and_update({"telegram_id": str(chat_id)}, {"$set": {"is_volunteer": True}})
    collection_name['users'].find().close()
